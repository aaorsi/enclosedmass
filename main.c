...
//   NSubVol  = atoi(argv[6]);
   ...
   while(!feof(fcen))
   {
        fscanf(fcen,"%d %f %f %f",&id_Central[j],&Central[j].x,\
...
//   for (iv = 0; iv < NSubVol; iv++)
   ...
   {
   sprintf(gadgetfile,"%s.%d.hdf5",gadget_,iv);
   printf("%s\n",gadgetfile);  
...
// Next Central particle in the linked list.
   ...
   for (i = 0; i<NCentral; i++)
   {
      fprintf(fout,"%d\n",id_Central[i]);
      for (j = 0;j<NBins;j++)
  	     fprintf(fout,"%f %f\n",pow(10,RMin + BinSize*j),MassEnclosed[i][j]);
   }
    fclose(fout);
    fclose(fcen);
